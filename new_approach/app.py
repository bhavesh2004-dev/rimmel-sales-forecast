...
def load_data_from_sqlite():
    ...
    # Ensure exact 8 columns exist
    exact_cols = ['product', 'dates', 'predicted value', 'actual value', 'percentage error', 'lower bound', 'upper bound', 'tier category']
    for col in exact_cols:
        if col not in df_fore.columns:
            df_fore[col] = ''
            
    df_fore = df_fore[exact_cols]
...
def generate_exact_excel_report(df_sub):
    exact_cols = ['product', 'dates', 'predicted value', 'actual value', 'percentage error', 'lower bound', 'upper bound', 'tier category']
    report_df  = df_sub[exact_cols].sort_values(by='predicted value', ascending=False)
...
st.markdown('<div class="sub-header">Powered by 3-Tier Master Production Engine (forecast_aug_2026_3tier_master)</div>', unsafe_allow_html=True)
...
if df_forecasts is not None and df_history is not None:
    ...
    tier_engine_name = str(sku_forecast.get('tier category', '3-Tier Master Engine'))

    pred_units  = int(sku_forecast['predicted value'])
    act_units   = int(sku_forecast['actual value']) if pd.notnull(sku_forecast['actual value']) and sku_forecast['actual value'] != '' else 0
    ...
    with tab3:
        ...
        with d2:
            exact_columns = ['product', 'dates', 'predicted value', 'actual value', 'percentage error', 'lower bound', 'upper bound', 'tier category']
