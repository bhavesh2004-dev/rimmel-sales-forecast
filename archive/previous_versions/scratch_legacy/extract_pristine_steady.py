"""
DEEP FORENSIC SCAN: EXTRACT ONLY PRISTINE HIGH-ACCURACY STEADY PRODUCTS
=======================================================================
Filter Rules:
1. Active Products: Must have real sales in 2026 and positive July actuals (act_jul > 0, tot_sales >= 100)
2. Steady Demand: Weekly CV <= 0.85, regular sales across the year
3. High Accuracy in July Test:
   - High volume (> 300 units): Accuracy >= 88% (e.g. RIM-MSC-ESL-101 diff 10, RIM-SMPP-001 diff 51)
   - Moderate volume (50-300 units): Difference <= 25 units AND Accuracy >= 82%
   - Lower volume (< 50 units): Difference <= 8 units
4. NO Major Discrepancy Products (Strictly excludes any SKU with large error!)
5. Clean 5 Columns in Descending Order of Sales
"""
import os, sqlite3
import pandas as pd
import numpy as np
...
OUT_FILE = os.path.join(BASE_DIR, 'reports', 'Rimmel_Pristine_Steady_Products_Forecast.xlsx')
...
for sku in all_588_skus:
    ...
    # Must be an active product with real sales (no 0-sales lines)
    ...
    # ── STRICT HIGH-ACCURACY FILTER ──────────────────────────────────────────
    # 1. Steady demand behavior all year round (CV <= 0.85, regular sales)
    # 2. High Accuracy thresholds:
    #    - For High Volume (> 300 units): Accuracy >= 88% and Diff <= 60
    #    - For Medium Volume (50 to 300 units): Diff <= 25 units AND Accuracy >= 80%
    #    - For Low Volume (< 50 units): Diff <= 10 units AND Accuracy >= 75%
    is_steady = (cv_weekly <= 0.88) and (ins_rate >= 0.45) and (active_days_ratio >= 0.20)
    
    if act_jul >= 300:
        is_pristine = (acc_jul >= 0.88) and (diff_jul <= 60)
    elif act_jul >= 50:
        is_pristine = (diff_jul <= 25) and (acc_jul >= 0.80)
    else:
    ...
    pristine_steady_records.append({
        'product': sku,
...
df_pristine_sorted = pd.DataFrame(pristine_steady_records).sort_values(
...
for target_f in [OUT_FILE, OUT_ACTIVE]:
    try:
        writer = pd.ExcelWriter(target_f, engine='xlsxwriter')
        df_client_clean.to_excel(writer, sheet_name='Pristine Steady Forecast', index=False)
        wb = writer.book
        ...
        ws = writer.sheets['Pristine Steady Forecast']
...
print("="*110)
print("TOP 30 PRODUCTS (WITH DIFF & ACCURACY AUDIT):")
print(df_pristine_sorted[['product', 'actual value (21/07/2026 to 31/07/2026)', 'predicted value (21/07/2026 to 31/07/2026)', '_diff', '_acc', 'predicted value (01/08/2026 to 11/08/2026)']].head(30).to_string(index=False))
